...
#with open('../youdao_course1.json', 'r') as f:
#    aaa = f.readlines()
conn = redis.Redis('r-2zewk8zid8qy6hj58k.redis.rds.aliyuncs.com', port=6379, password='GYqOtg2ovIDLk67Sb3')

print(conn.llen('koolearn:urls'))
#conn.rpop('http_proxy')
print(conn.lrange('http_proxy', 0, -1))
print(conn.lrange('https_proxy', 0, -1))
#conn.lrem('http_proxy', '1', '183.165.32.178:4203_1608171880.1295671_110')
print(len(conn.smembers('youdao_course_id')))

#conn.delete('https_proxy')
#conn.delete('http_proxy')
#for i in conn.lrange('youdao:urls', 0, -1):
#    if i.decode() == 'https://ke.youdao.com/course/api/detail.json?courseId=77007':
#        print('aaaaaa')
#        print(i.decode())
#        break
#conn.lpush('koolearn:urls', 'http://study.koolearn.com/api/product/?productIds=80991')
#for i in aaa:
#    id = json.loads(i)['course_id']
#    #conn.lpush('koolearn:urls', f'http://study.koolearn.com/api/product/?productIds={id}')
#    conn.sadd('youdao_course_id', id)
#    print(id)

# subprocess.run(['scrapy', 'runspider', 'myspider/spiders/koolearn/koolearn_sell.py'])
import time
# def test():
#     print(1111222)
#
# sched = BlockingScheduler()
# sched.add_job(test, 'cron', hour=18, minute=37, second=00)
# sched.start()
from happybase_kerberos_patch import KerberosConnection, KerberosConnectionPool
##connection = KerberosConnection('192.168.1.173', protocol='compact', use_kerberos=True)
connection = KerberosConnection('kc-bigdata-03.bj02', protocol='compact', use_kerberos=True, table_prefix='competitor', table_prefix_separator=':', sasl_service_name='hbase')
#schema = {
#                'info': dict(max_versions=3)
#            }

print(connection.tables())
#connection.create_table('course_detail', schema)
#print(connection.tables())
#tb = connection.table('competitor:course_info')
#batch = tb.batch()
#scan = tb.scan(limit=2)
#print(scan)
